---
title: "Broker: sharing coworkers"
...
- [The three coworkers in personas/](#the-three-coworkers-in-personas)
...
gallery install path fetches nothing else). See `personas/` for three worked
examples, all of which parse cleanly under the client's strict parser.
...
| `risk_summary`           | derived from `tools:` / connectors / `messaging:`                 |
...
```
remit-broker gallery --config broker.toml list
remit-broker gallery --config broker.toml show <id>
remit-broker gallery --config broker.toml approve <id>
remit-broker gallery --config broker.toml reject <id> [why]
remit-broker gallery --config broker.toml journal
remit-broker gallery --config broker.toml verify
```
...
notes-to-confluence      from ada@example.com      2026-09-01T02:06:43Z
    skills: triage.md
    read it: remit-broker gallery --config broker.toml show notes-to-confluence

1 waiting. approve with:  remit-broker gallery --config broker.toml approve <id>
```

`show` prints the thing itself — the manifest, then every skill:

```
$ remit-broker gallery --config broker.toml show notes-to-confluence
notes-to-confluence
...
```
remit-broker gallery --config broker.toml approve notes-to-confluence
kill -HUP $(pgrep -f remit-broker)
```
...
## The three coworkers in `personas/`

`appsec-reviewer`, `incident-scribe` and `onboarding-guide` are **samples**, so
that a fresh broker has something to serve and a manifest to copy.

They are also real gallery entries. A deployment that leaves `gallery.dir`
pointed at `./personas` publishes all three to the whole company, under
publishers nobody there chose ("Platform Security", "Developer Experience"),
and they sit alongside real work in everyone's gallery.
