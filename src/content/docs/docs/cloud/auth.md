...
- [Operators](/docs/cloud/sharing/#reviewing-without-a-shell) — who may administer the
...
```toml
[tls]
cert_file = "/etc/remit-cloud/tls/fullchain.pem"
key_file  = "/etc/remit-cloud/tls/privkey.pem"
```
...
The broker reads `email` and `name` from the access token. Not every provider
puts them there: Auth0 leaves profile claims out of API access tokens unless the
tenant adds an Action, and Entra sends `preferred_username`. Point `userinfo_url`
at the provider's userinfo endpoint (from its discovery document) and the broker
asks it, with the bearer, whenever a token has no `email` — once per token, cached
until the token expires, and only trusted when the answer's `sub` is the token's.
A token that already names its person is never looked up. A userinfo endpoint
that is down leaves the session valid and nameless, not refused.

```toml
[auth.oidc]
userinfo_url = "https://tenant.us.auth0.com/userinfo"
```

#### An address the provider has not verified

A self-service sign-up holds a token before anyone clicks the confirmation link,
and the provider says so: `email_verified: false`, in the token or from userinfo.
The broker reports that address and attributes to it, but it **establishes**
nothing: not on the operator allowlist, not an owner of an upload. Otherwise an
open sign-up tenant plus an operator list matched on email would let anyone
become an operator by typing the operator's address. A provider that says nothing
(Okta and Entra access tokens carry no such claim) has asserted the address, and
it counts. `/v1/admin/whoami` names this case when it is the reason.
