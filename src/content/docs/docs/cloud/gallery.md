---
title: "The gallery"
description: "> This is the standalone broker's README as it stood on 2026-09-02, kept because its deployment > walkthrough still applies to the gallery subsystem…"
---
...
> This is the standalone broker's README as it stood on 2026-09-02, kept because its deployment
> walkthrough still applies to the gallery subsystem inside `remit-cloud`. Binary and config names
> below have been updated; the standalone product lives on at Remit Coworker
...
**USER_GUIDE.md** covers adding and sharing coworkers.
...
| docs/auth.md | auth modes, TLS, sign-in codes, operators |
| docs/sharing.md | what travels, uploads, review, the journal |
| docs/api.md | every endpoint and its shape |
| USER_GUIDE.md | for whoever curates the gallery |

## Quick start

```sh
make build
./bin/remit-cloud            # :8443, ./personas, auth mode `none`, no TLS
...
listen     = "127.0.0.1:8443"                  # behind a proxy; ":8443" to face the network
                                               # (the hosted control plane runs on 443 by
                                               # socket activation — docs/deployment.md)
...
it behind a proxy that has one. See TLS is required in practice
...
requiring a sign-in code for what it
...
In each worker's `config.toml` (`~/.config/coworker/config.toml`), or from the
app's Settings pane:
...
In the client's `config.toml` (`<state-dir>/config.toml`), or from the app's
Settings pane. These keys are file-only: the client reads no environment
override for them.
...
Each scalar has a `REMIT_CLOUD_<PATH>` twin:
...
```sh
REMIT_CLOUD_LISTEN=:9443 \
REMIT_CLOUD_AUTH_MODE=oidc \
REMIT_CLOUD_AUTH_OIDC_ISSUER=https://corp.okta.com/oauth2/default \
REMIT_CLOUD_GALLERY_DIR=/srv/personas \
  ./bin/remit-cloud
```

```
remit-cloud --config remit-cloud.toml [--log-level info] [--version]
```

## Development

```
make build    # -> bin/remit-cloud
make test     # go test ./...
...
```
cmd/remit-cloud/   the binary: config, signals, graceful shutdown
